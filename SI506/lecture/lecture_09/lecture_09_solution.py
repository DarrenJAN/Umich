...
# um_count_48104 = 0
# for station in stations:
#     if station[1].startswith('U-M') and int(station[4]) == 48104:
#         um_count_48104 += 1
...
i = 0
while i < len(stations):
    if stations[i][1].startswith('U-M') and 'greene st' in stations[i][3].lower():
        um_stations_greene_st.append(stations[i])
    i += 1
...
# conv_stores = []
# for station in stations:
#     if 'meijer' in station[1].lower() or station[2].lower() == 'convenience_store':
#         conv_stores.append(station)
...
i = 0
while i < len(stations):
    if (stations[i][1].startswith('Shell') or
        'meijer' in stations[i][1].lower() or
        stations[i][2].lower() == 'convenience_store'):
        conv_stores.append(stations[i][1])
    i += 1
...
# conv_stores = []
# for station in stations:
#     if (station[1].startswith('Shell') or
#         'meijer' in station[1].lower() or
#         station[2].lower() == 'convenience_store'):
#         conv_stores.append(station)
...
# station_count = 0
# for station in stations:
#     if station[headers.index('ev_network')] == 'ChargePoint Network':
#         station_count += 1
...
ev_conn_type_vals = []
for station in stations:
    if station[headers.index('ev_connector_types')] not in ev_conn_type_vals:
        ev_conn_type_vals.append(station[headers.index('ev_connector_types')])

print(f"EV connector type values = {ev_conn_type_vals}")

station_count = 0
i = 0
while i < len(stations):
    if not 'J1772' in stations[i][headers.index('ev_connector_types')]:
        # print(f"{stations[i][1]} {stations[i][headers.index('ev_connector_types')]}")
        station_count += 1
    i += 1
...
facility_type_vals = []
for station in stations:
    if station[2] not in facility_type_vals:
        facility_type_vals.append(station[2])

print(f"\n1.4.1 facility_type unique values (n={len(facility_type_vals)}) = {facility_type_vals}")
...
# Alternative (for loop)
# for station in stations:
#     if station[idx].lower() == 'chargepoint network':
#         chargepoint_count += 1
#     elif station[idx].lower() == 'ev connect':
#         ev_connect_count += 1
#     elif station[idx].lower() == 'evgo network':
#         evgo_count += 1
#     elif station[idx].lower() == 'greenlots':
#         greenlots_count += 1
#     else:
...
for station in stations:
    ...
    elif station[idx].lower() == 'non-networked':
        non_network_count += 1
    else:
        network_count += 1
...
# Alternative: Truth value testing (covered in a later lecture)
# network_count = 0
# non_network_count = 0
# network_unknown_count = 0
# idx = headers.index('ev_network') # lookup index value
...
# for station in stations:
#     if station[idx] and station[idx].lower() == 'non-networked':
#         non_network_count += 1
#     elif station[idx] and not station[idx].lower() == 'non-networked':
#         network_count += 1
#     else:
#         network_unknown_count += 1

# # Pass multiple strings
# print(f"\n2.0.2 Ann Arbor EV charging stations",
#     f"\nIn network count = {network_count}",
#     f"\nNon-network count = {non_network_count}",
#     f"\nUnknown count = {network_unknown_count}")
