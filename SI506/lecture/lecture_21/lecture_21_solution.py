...
def main():
    ...
    china_pop_indicators = {
        ...
        'population_indicators': {k: v for k, v in china['country_indicators'].items() if k[3:6] == 'POP'}
    }
    ...
    east_asia_pop_indicators = {}
    for group in groups:
        if group['group_code'] == 'EAS':
            for country in country_indicators:
                if group['country_code'] == country['country_code']:
                    east_asia_pop_indicators[country['country_code']] = {
                        'country_code': country['country_code'],
                        'country_name': country['country_name'],
                        'population_indicators': {k: v for k, v in country['country_indicators'].items() if k[3:6] == 'POP'}
                    }

    print(f"\nCh 03 for loop: East Asia and Pacific country indicators (n={len(east_asia_pop_indicators)})")

    # Write to file
    write_json('./stu-east_asia-pop_indicators-for.json', east_asia_pop_indicators)
